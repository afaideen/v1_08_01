/*******************************************************************************
 Board initialization file for Bluetooth Audio Development Board (BTAD)

 Company:
    Microchip Technology Inc.

 File Name:
    bsp_sys_init.c

 Summary:
    Board Support Package implementation for Bluetooth Audio Development Kit with
	PIC32MZ Audio PIM.

 Description:
    This file contains the initialization of board specific I/O.
 *******************************************************************************/

// DOM-IGNORE-BEGIN
/*******************************************************************************
Copyright (c) 2014 released Microchip Technology Inc.  All rights reserved.

Microchip licenses to you the right to use, modify, copy and distribute
Software only when embedded on a Microchip microcontroller or digital signal
controller that is integrated into your product or third party product
(pursuant to the sublicense terms in the accompanying license agreement).

You should refer to the license agreement accompanying this Software for
additional information regarding your rights and obligations.

SOFTWARE AND DOCUMENTATION ARE PROVIDED AS IS WITHOUT WARRANTY OF ANY KIND,
EITHER EXPRESS OR IMPLIED, INCLUDING WITHOUT LIMITATION, ANY WARRANTY OF
MERCHANTABILITY, TITLE, NON-INFRINGEMENT AND FITNESS FOR A PARTICULAR PURPOSE.
IN NO EVENT SHALL MICROCHIP OR ITS LICENSORS BE LIABLE OR OBLIGATED UNDER
CONTRACT, NEGLIGENCE, STRICT LIABILITY, CONTRIBUTION, BREACH OF WARRANTY, OR
OTHER LEGAL EQUITABLE THEORY ANY DIRECT OR INDIRECT DAMAGES OR EXPENSES
INCLUDING BUT NOT LIMITED TO ANY INCIDENTAL, SPECIAL, INDIRECT, PUNITIVE OR
CONSEQUENTIAL DAMAGES, LOST PROFITS OR LOST DATA, COST OF PROCUREMENT OF
SUBSTITUTE GOODS, TECHNOLOGY, SERVICES, OR ANY CLAIMS BY THIRD PARTIES
(INCLUDING BUT NOT LIMITED TO ANY DEFENSE THEREOF), OR OTHER SIMILAR COSTS.
*******************************************************************************/
// DOM-IGNORE-END

// *****************************************************************************
// *****************************************************************************
// Section: Included Files
// *****************************************************************************
// *****************************************************************************

#include "bsp_config.h"

// *****************************************************************************
// *****************************************************************************
// *****************************************************************************
// Section: Interface Routines
// *****************************************************************************
// *****************************************************************************

// *****************************************************************************
/* Function:
    void BSP_Initialize(void)

  Summary:
    Performs the necessary actions to initialize a board

  Description:
    This function initializes the LED and Switch ports on the board. It disabled
    JTAG so that all LEDs connected to PORTA are available.  This function must
    be called by the user before using any APIs present on this BSP.

  Remarks:
    Refer to bsp_config.h for usage information.
*/

void BSP_Initialize(void )
{
    /* Disable JTAG so that all LED ports are available */
    PLIB_DEVCON_JTAGPortDisable(DEVCON_ID_0);

    PLIB_PORTS_PinClear ( PORTS_ID_0, PORT_CHANNEL_B, PORTS_BIT_POS_5 );

    /* Set the Initial state of LEDs */
    PLIB_PORTS_PinSet(PORTS_ID_0, PORT_CHANNEL_B, PORTS_BIT_POS_13);
    PLIB_PORTS_PinSet(PORTS_ID_0, PORT_CHANNEL_B, PORTS_BIT_POS_14);
    PLIB_PORTS_PinSet(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_2);
    PLIB_PORTS_PinSet(PORTS_ID_0, PORT_CHANNEL_H, PORTS_BIT_POS_12);
    PLIB_PORTS_PinSet(PORTS_ID_0, PORT_CHANNEL_J, PORTS_BIT_POS_13);

    /* Configure the ports for the USB switch */
    PLIB_PORTS_PinClear( PORTS_ID_0, PORT_CHANNEL_K, PORTS_BIT_POS_5 );
    PLIB_PORTS_PinClear( PORTS_ID_0, PORT_CHANNEL_K, PORTS_BIT_POS_4 );

    /* BT Pin */
    PLIB_PORTS_PinClear( PORTS_ID_0, PORT_CHANNEL_G, PORTS_BIT_POS_15 );
}

// *****************************************************************************
/* Function:
    void BSP_LEDStateSet(BSP_LED led, BSP_LED_STATE state);

  Summary:
    Controls the state of the LED.

  Description:
    This function allows the application to specify the state of the LED.

  Remarks:
    None.
*/

void BSP_LEDStateSet(BSP_LED led, BSP_LED_STATE state)
{
	if(BSP_LED_5 == led)
    PLIB_PORTS_PinWrite ( PORTS_ID_0 , PORT_CHANNEL_B , PORTS_BIT_POS_13, state );
	else if(BSP_LED_6 == led)
    PLIB_PORTS_PinWrite ( PORTS_ID_0 , PORT_CHANNEL_B , PORTS_BIT_POS_14, state );
	else if(BSP_LED_7 == led)
    PLIB_PORTS_PinWrite ( PORTS_ID_0 , PORT_CHANNEL_F , PORTS_BIT_POS_2, state );
	else if(BSP_LED_8 == led)
    PLIB_PORTS_PinWrite ( PORTS_ID_0 , PORT_CHANNEL_H , PORTS_BIT_POS_12, state );
	else if(BSP_LED_9 == led)
    PLIB_PORTS_PinWrite ( PORTS_ID_0 , PORT_CHANNEL_J , PORTS_BIT_POS_13, state );
}

/*******************************************************************************
  Function: void BSP_USBSwitchSelect(void)

  Summary:
   Sets the USB Switch to define USB Mode to be Device or Host.

  Description:
    Sets the USB Switch to define USB Mode to be Device or Host.

  Remarks:
    None.

*/
void BSP_USBSwitchSelect(uint32_t value)
{
    PLIB_PORTS_Write(PORTS_ID_0, PORT_CHANNEL_K, value);
}

// *****************************************************************************
/* Function:
    void BSP_LEDOn(BSP_LED led);

  Summary:
    Switches ON the specified LED.

  Description:
    This function switches ON the specified LED.

  Remarks:
    None.
*/

void BSP_LEDOn(BSP_LED led)
{
	if(BSP_LED_5 == led)
	PLIB_PORTS_PinClear( PORTS_ID_0, PORT_CHANNEL_B, PORTS_BIT_POS_13);
	else if(BSP_LED_6 == led)
	PLIB_PORTS_PinClear( PORTS_ID_0, PORT_CHANNEL_B, PORTS_BIT_POS_14);
	else if(BSP_LED_7 == led)
	PLIB_PORTS_PinClear( PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_2);
	else if(BSP_LED_8 == led)
	PLIB_PORTS_PinClear( PORTS_ID_0, PORT_CHANNEL_H, PORTS_BIT_POS_12);
	else if(BSP_LED_9 == led)
	PLIB_PORTS_PinClear( PORTS_ID_0, PORT_CHANNEL_J, PORTS_BIT_POS_13);
}

// *****************************************************************************
/* Function:
    void BSP_LEDOff(BSP_LED led);

  Summary:
    Switches OFF the specified LED.

  Description:
    This function switches OFF the specified LED.

  Remarks:
    None.
*/

void BSP_LEDOff(BSP_LED led)
{
	if(BSP_LED_5 == led)
	PLIB_PORTS_PinSet( PORTS_ID_0, PORT_CHANNEL_B, PORTS_BIT_POS_13);
	else if(BSP_LED_6 == led)
	PLIB_PORTS_PinSet( PORTS_ID_0, PORT_CHANNEL_B, PORTS_BIT_POS_14);
	else if(BSP_LED_7 == led)
	PLIB_PORTS_PinSet( PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_2);
	else if(BSP_LED_8 == led)
	PLIB_PORTS_PinSet( PORTS_ID_0, PORT_CHANNEL_H, PORTS_BIT_POS_12);
	else if(BSP_LED_9 == led)
	PLIB_PORTS_PinSet( PORTS_ID_0, PORT_CHANNEL_J, PORTS_BIT_POS_13);
}

// *****************************************************************************
/* Function:
    BSP_LED_STATE BSP_LEDStateGet(BSP_LED led);

  Summary:
    Returns the present state of the LED.

  Description:
    This function returns the present state of the LED.

  Remarks:
    None.
*/

BSP_LED_STATE BSP_LEDStateGet(BSP_LED led)
{
	if(BSP_LED_5 == led)
    return PLIB_PORTS_PinGetLatched(PORTS_ID_0, PORT_CHANNEL_B, PORTS_BIT_POS_13);
	else if(BSP_LED_6 == led)
    return PLIB_PORTS_PinGetLatched(PORTS_ID_0, PORT_CHANNEL_B, PORTS_BIT_POS_14);
	else if(BSP_LED_7 == led)
    return PLIB_PORTS_PinGetLatched(PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_2);
	else if(BSP_LED_8 == led)
    return PLIB_PORTS_PinGetLatched(PORTS_ID_0, PORT_CHANNEL_H, PORTS_BIT_POS_12);
	else if(BSP_LED_9 == led)
    return PLIB_PORTS_PinGetLatched(PORTS_ID_0, PORT_CHANNEL_J, PORTS_BIT_POS_13);

    return false;
}

// *****************************************************************************
/* Function:
    void BSP_LEDToggle(BSP_LED led);

  Summary:
    Toggles the state of the LED between BSP_LED_STATE_ON and BSP_LED_STATE_OFF.

  Description:
    This function toggles the state of the LED between BSP_LED_STATE_ON and
    BSP_LED_STATE_OFF.

  Remarks:
    None.
*/

void BSP_LEDToggle(BSP_LED led)
{
	if(BSP_LED_5 == led)
	PLIB_PORTS_PinToggle( PORTS_ID_0, PORT_CHANNEL_B, PORTS_BIT_POS_13);
	else if(BSP_LED_6 == led)
	PLIB_PORTS_PinToggle( PORTS_ID_0, PORT_CHANNEL_B, PORTS_BIT_POS_14);
	else if(BSP_LED_7 == led)
	PLIB_PORTS_PinToggle( PORTS_ID_0, PORT_CHANNEL_F, PORTS_BIT_POS_2);
	else if(BSP_LED_8 == led)
	PLIB_PORTS_PinToggle( PORTS_ID_0, PORT_CHANNEL_H, PORTS_BIT_POS_12);
	else if(BSP_LED_9 == led)
	PLIB_PORTS_PinToggle( PORTS_ID_0, PORT_CHANNEL_J, PORTS_BIT_POS_13);
}

// *****************************************************************************
/* Function:
    void BSP_SwitchStateGet(BSP_SWITCH switch);

  Summary:
    Returns the present state (pressed or not pressed) of the specified switch.

  Description:
    This function returns the present state (pressed or not pressed) of the
    specified switch.

  Remarks:
    None.
*/

BSP_SWITCH_STATE BSP_SwitchStateGet( BSP_SWITCH bspSwitch )
{
    if(bspSwitch == BSP_SWITCH_1)
    return PLIB_PORTS_PinGet(PORTS_ID_0, PORT_CHANNEL_K, PORTS_BIT_POS_0);
    else if(bspSwitch == BSP_SWITCH_2)
    return PLIB_PORTS_PinGet(PORTS_ID_0, PORT_CHANNEL_A, PORTS_BIT_POS_10);
	else if(bspSwitch == BSP_SWITCH_3)
    return PLIB_PORTS_PinGet(PORTS_ID_0, PORT_CHANNEL_J, PORTS_BIT_POS_14);
    else if(bspSwitch == BSP_SWITCH_4)
    return PLIB_PORTS_PinGet(PORTS_ID_0, PORT_CHANNEL_H, PORTS_BIT_POS_0);
    else if(bspSwitch == BSP_SWITCH_5)
    return PLIB_PORTS_PinGet(PORTS_ID_0, PORT_CHANNEL_H, PORTS_BIT_POS_1);

    return false;
}

// *****************************************************************************
/* Function:
    void BSP_BluetoothPinStateSet(BSP_BT_STATE state);

  Summary:
    Sets to low/high the pin to the Bluetooth module.

  Description:
    Sets to low/high the pin to the Bluetooth module.

  Remarks:
    None.
*/
void BSP_BluetoothPinStateSet(BSP_BT_STATE state)
{
    PLIB_PORTS_PinWrite (PORTS_ID_0 , PORT_CHANNEL_G , PORTS_BIT_POS_15, state );
}

// *****************************************************************************
/* Function:
    bool BSP_USBVBUSOverCurrentDetect(uint8_t port)

  Summary:
    Returns true if the over current is detected on the VBUS supply.

  Description:
    This function returns true if over current is detected on the VBUS supply.

  Remarks:
    None.
*/

bool BSP_USBVBUSSwitchOverCurrentDetect(uint8_t port)
{
    return(false);
}

// *****************************************************************************
/* Function:
    bool BSP_USBVBUSPowerEnable(uint8_t port, bool enable)

  Summary:
    This function controls the USB VBUS supply.

  Description:
    This function controls the USB VBUS supply.

  Remarks:
    None.
*/

void BSP_USBVBUSPowerEnable(uint8_t port, bool enable)
{
    if(enable)
    {
        PLIB_PORTS_PinSet(PORTS_ID_0, PORT_CHANNEL_E, PORTS_BIT_POS_8);
    }
    else
    {
        PLIB_PORTS_PinClear(PORTS_ID_0, PORT_CHANNEL_E, PORTS_BIT_POS_8);
    }
}

/*******************************************************************************
 End of File
*/
