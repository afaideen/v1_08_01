/*******************************************************************************
  ...
  Summary:
    ...
    Kit in combination with the 169lfbga_cpu daughter card and the Multimedia 
    Expansion Board II and a WVGA display.

  Description:
    This file contains routines that implement the Board Support Package for the
    PIC32MZ Graphics (DA) Starter Kit in combination with the 169lfbga_cpu 
    daughter cardMultimedia and the Expansion Board II and a WVGA display.
*******************************************************************************/
...
{
    PORTS_CHANNEL bspPort;

    if(bspSwitch == BSP_SWITCH_S1)
    {
        bspPort = PORT_CHANNEL_A;
    }
    else
    {
        bspPort = PORT_CHANNEL_B;
    }

    return ( PLIB_PORTS_PinGet(PORTS_ID_0, bspPort, bspSwitch) );
}
