 ...
 ****************************************/
SYS_FS_MEDIA_GEOMETRY *gDrvSPIFlashMediaGeometry;
SYS_FS_MEDIA_REGION_GEOMETRY *gSPIFlashGeometryTable;
...
const SYS_FS_MEDIA_FUNCTIONS spiflashMediaFunctions =
{
    .mediaStatusGet     = DRV_SST25VF016B_MediaIsAttached,
    .mediaGeometryGet   = DRV_SST25VF016B_GeometryGet,
    .sectorRead         = DRV_SST25VF016B_BlockRead,
    .sectorWrite        = DRV_SST25VF016B_BlockWrite,  //_DRV_SPIFLASH_ERASE_WRITE_WRAPPER,
    .eventHandlerset    = DRV_SST25VF016B_BlockEventHandlerSet,
    .commandStatusGet   = (void *)DRV_SST25VF016B_CommandStatus,
//    .Read               = DRV_SPIFLASH_Read,
    .Read               = (void *)SPIFlashReadArray,//(void *)DRV_SST25VF016B_BlockRead,
    .Write              = (void *)SPIFlashWriteArray,
    .erase              = DRV_SST25VF016B_BlockErase,
...
// #if defined(DRV_SST25VF_SYS_FS_REGISTER)   
    /* Set the current driver state */
    dObj->status = SYS_STATUS_READY;
    dObj->blockStartAddress = sst25vf016bInit->mediaStartAddress;//MPFS_RESERVE_BLOCK;

    _DRV_SST25VF016B_RegisterWithSysFs(dObj, drvIndex, spiflashMediaFunctions);
              ...
              if (!(hDriver->commandAddressData[3] & 0x01))
              {
                    ...
                    else
                    {
                        ...
                        * 1 erase block = 4kbyte = 0x1000 */
                        ...
                        bufferObj->status  = DRV_SPIFLASH_COMMAND_COMPLETED; 
                    }
                    /* re-initialize the internal buffer processing state for the new buffer object */
                    hDriver->bufferProcessState = DRV_SST25VF016B_BUFFER_PROCESS_INIT;
              }
...
)
{
    ...
    for(i = 0 ; i < DRV_SST25VF016B_QUEUE_DEPTH_COMBINED; i ++)
    {
        if(!gDrvSST25VF016BBufferObj[i].inUse)
        {
            ...
            bufferObj->status = DRV_SPIFLASH_COMMAND_QUEUED;
...
uintptr_t DRV_SST25VF016B_AddressGet
(
    const DRV_HANDLE handle
)
{
    ...
    /* Validate the handle */
    _DRV_SST25VF016B_VALIDATE_EXPR((0 == handle), (uintptr_t)NULL);
    _DRV_SST25VF016B_VALIDATE_EXPR((DRV_HANDLE_INVALID == handle), (uintptr_t)NULL);

    /* See if the client has been opened */
    clientObj = (DRV_SST25VF016B_CLIENT_OBJ *)handle;
    _DRV_SST25VF016B_VALIDATE_EXPR((!clientObj->inUse), (uintptr_t)NULL);

    /* Check if the driver is ready for operation */
    dObj = (DRV_SST25VF016B_OBJ *)clientObj->hDriver;//driverObj used in NVM;
//    dObj = &gDrvSST25VF016BObj[0];  //(DRV_SST25VF016B_OBJ *)clientObj->driverObj;
    SYS_STATUS val;
    val = dObj->status;
    _DRV_SST25VF016B_VALIDATE_EXPR((dObj->status != SYS_STATUS_READY), (uintptr_t)NULL);

    return dObj->blockStartAddress;
}

DRV_SPIFLASH_COMMAND_STATUS DRV_SST25VF016B_CommandStatus
(
    const DRV_HANDLE handle,
    const DRV_SPIFLASH_COMMAND_HANDLE commandHandle
)
{
    uint16_t iEntry;

    /* Validate the client handle */
    _DRV_SST25VF016B_VALIDATE_EXPR((NULL == _DRV_SST25VF016B_DriverHandleValidate(handle)), DRV_SST25VF016B_BLOCK_COMMAND_HANDLE_INVALID);

    /* The upper 16 bits of the buffer handle
     * are the token and the lower 16 bits of the
     * are buffer index into the gDrvSST25VF016BBufferObj
     * array */
//    iEntry = commandHandle & 0xFFFF;
    iEntry = commandHandle / (uint32_t)&gDrvSST25VF016BBufferObj[0];
    Nop();
    ...
    #else
    if(gDrvSST25VF016BBufferObj[iEntry].commandHandle != commandHandle)
    {
        /* This means that object has been re-used by another
         * request. Indicate that the operation is completed.
         * */
        return (DRV_SPIFLASH_COMMAND_COMPLETED);
    }
