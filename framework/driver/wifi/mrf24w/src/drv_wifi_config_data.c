...
static bool LoadConfigFromMemory(void)
{
    ...
    if(memcmp(appData.config.wifi.ssid, (ROM void*)DRV_WIFI_DEFAULT_SSID, sizeof(DRV_WIFI_DEFAULT_SSID)) 
            && appData.config.wifi.ssidLen )
    {
        SYS_CONSOLE_MESSAGE("Load stored Wi-Fi configuration\r\n");
        memcpy((uint8_t *)p_wifi_configData, (uint8_t *)&appData.config.wifi, sizeof(DRV_WIFI_CONFIG_DATA));
...
{
#if defined(DRV_SST25VF016B_CLIENTS_NUMBER)
    memcpy((uint8_t *)&appData.config.wifi, (uint8_t *)p_wifi_configData, sizeof(DRV_WIFI_CONFIG_DATA));
...
//    SPIFlashEraseSector(0);
    memset(&appData.config.wifi, 0, sizeof(DRV_WIFI_CONFIG_DATA));
    SaveAppConfig(&appData.config.wifi, SPIFLASH_APPCONFIG_WIFI_ADDRESS);
    SYS_CONSOLE_MESSAGE("***************************************\r\n"
