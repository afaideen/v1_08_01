...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _SB_CFGCON_DMAPRI_VREG(index)

  MASKs: 
    _SB_CFGCON_DMAPRI_MASK(index)

  POSs: 
    _SB_CFGCON_DMAPRI_POS(index)

  LENs: 
    _SB_CFGCON_DMAPRI_LEN(index)
...
{
    _SFR_BIT_WRITE(_SB_CFGCON_DMAPRI_VREG(index), _SB_CFGCON_DMAPRI_POS(index), priority);
}
