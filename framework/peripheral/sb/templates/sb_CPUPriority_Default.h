...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _SB_CFGCON_EXLPRI_VREG(index)

  MASKs: 
    _SB_CFGCON_EXLPRI_MASK(index)

  POSs: 
    _SB_CFGCON_EXLPRI_POS(index)

  LENs: 
    _SB_CFGCON_EXLPRI_LEN(index)
...
{
    _SFR_BIT_WRITE(_SB_CFGCON_EXLPRI_VREG(index), _SB_CFGCON_EXLPRI_POS(index), priority);
}
