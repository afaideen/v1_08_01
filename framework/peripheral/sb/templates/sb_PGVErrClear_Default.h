...
{
    return (PLIB_SB_ERROR)_SFR_FIELD_READ(_SB_T0_CODE_VREG(index) + ((0x400 * target) >> 2), _SB_T0_CODE_MASK(index), _SB_T0_CODE_POS(index));
}
...
{
    _SFR_FIELD_WRITE(_SB_T0_CODE_VREG(index) + ((0x400 * target) >> 2), _SB_T0_CODE_MASK(index), _SB_T0_CODE_POS(index), 0x3); 
}
