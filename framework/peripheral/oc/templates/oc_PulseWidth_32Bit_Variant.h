...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _OC_SEC_CMP_VAL_VREG(index)

  MASKs: 
    _OC_SEC_CMP_VAL_MASK(index)

  POSs: 
    _OC_SEC_CMP_VAL_POS(index)

  LENs: 
    _OC_SEC_CMP_VAL_LEN(index)
...
{
    _SFR_WRITE(_OC_SEC_CMP_VAL_VREG(index),pulseWidth);
}
...
{
    _SFR_WRITE(_OC_SEC_CMP_VAL_VREG(index), (pulseWidth & 0X0000ffff));
}
