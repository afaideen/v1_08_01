...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _OC_FAULT_INPUT_VREG(index)

  MASKs: 
    _OC_FAULT_INPUT_MASK(index)

  POSs: 
    _OC_FAULT_INPUT_POS(index)

  LENs: 
    _OC_FAULT_INPUT_LEN(index)
...
{
    _SFR_FIELD_WRITE(_OC_FAULT_INPUT_VREG(index),_OC_FAULT_INPUT_MASK(index),_OC_FAULT_INPUT_POS(index), flt);
}
