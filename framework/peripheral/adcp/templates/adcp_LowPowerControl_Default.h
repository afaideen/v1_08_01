...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _ADCP_LOW_POWER_ENABLE_VREG(index)

  MASKs: 
    _ADCP_LOW_POWER_ENABLE_MASK(index)

  POSs: 
    _ADCP_LOW_POWER_ENABLE_POS(index)

  LENs: 
    _ADCP_LOW_POWER_ENABLE_LEN(index)
...
{
    _SFR_BIT_WRITE(_ADCP_LOW_POWER_ENABLE_VREG(index),_ADCP_LOW_POWER_ENABLE_POS(index),1);
}
...
{
    _SFR_BIT_WRITE(_ADCP_LOW_POWER_ENABLE_VREG(index),_ADCP_LOW_POWER_ENABLE_POS(index),0);
}
...
{
    return _SFR_BIT_READ(_ADCP_LOW_POWER_ENABLE_VREG(index),
            _ADCP_LOW_POWER_ENABLE_POS(index) );
}
