...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _EBI_EBIRDYEN2_VREG(index)
    _EBI_EBIRDYINV2_VREG(index)

  MASKs: 
    _EBI_EBIRDYEN2_MASK(index)
    _EBI_EBIRDYINV2_MASK(index)

  POSs: 
    _EBI_EBIRDYEN2_POS(index)
    _EBI_EBIRDYINV2_POS(index)

  LENs: 
    _EBI_EBIRDYEN2_LEN(index)
    _EBI_EBIRDYINV2_LEN(index)
...
{    
    _SFR_BIT_WRITE(_EBI_EBIRDYEN2_VREG(index), _EBI_EBIRDYEN2_POS(index), ReadyPin2Enable);
    _SFR_BIT_WRITE(_EBI_EBIRDYINV2_VREG(index), _EBI_EBIRDYINV2_POS(index), ReadyPin2Invert);
}
