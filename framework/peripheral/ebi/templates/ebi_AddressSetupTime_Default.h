...
PLIB_TEMPLATE int EBI_AddressSetupTimeGet_Default( EBI_MODULE_ID index , int ChipSelectNumber )
{
	...
	{
	case 0:
	return _SFR_FIELD_READ(_EBI_TAS0_VREG(index),_EBI_TAS0_MASK(index),_EBI_TAS0_POS(index));
	break;

	case 1:
	return _SFR_FIELD_READ(_EBI_TAS1_VREG(index),_EBI_TAS1_MASK(index),_EBI_TAS1_POS(index));
	break;

	case 2:
	return _SFR_FIELD_READ(_EBI_TAS2_VREG(index),_EBI_TAS2_MASK(index),_EBI_TAS2_POS(index));
	break;
