...
PLIB_TEMPLATE EBI_PAGE_SIZE EBI_MemoryPageSizeGet_Default( EBI_MODULE_ID index , int ChipSelectNumber )
{
	...
	return (EBI_PAGE_SIZE)_SFR_FIELD_READ(_EBI_PAGESIZE0_VREG(index), _EBI_PAGESIZE0_MASK(index), _EBI_PAGESIZE0_POS(index) );
	break;

	case 1:
	
	return (EBI_PAGE_SIZE)_SFR_FIELD_READ(_EBI_PAGESIZE1_VREG(index), _EBI_PAGESIZE1_MASK(index), _EBI_PAGESIZE1_POS(index) );
	break;
	
	case 2:
	
	return (EBI_PAGE_SIZE)_SFR_FIELD_READ(_EBI_PAGESIZE2_VREG(index), _EBI_PAGESIZE2_MASK(index), _EBI_PAGESIZE2_POS(index) );
	break;
