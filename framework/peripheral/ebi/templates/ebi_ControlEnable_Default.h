...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _EBI_EBIPINEN_VREG(index)

  MASKs: 
    _EBI_EBIPINEN_MASK(index)

  POSs: 
    _EBI_EBIPINEN_POS(index)

  LENs: 
    _EBI_EBIPINEN_LEN(index)
...
{
    _SFR_BIT_WRITE(_EBI_EBIPINEN_VREG(index),  _EBI_EBIPINEN_POS(index), EnableBit);
}
...
{
    return (_SFR_BIT_READ(_EBI_EBIPINEN_VREG(index),  _EBI_EBIPINEN_POS(index)));
}
