...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _EBI_TRPD_VREG(index)

  MASKs: 
    _EBI_TRPD_MASK(index)
...
{
    _SFR_FIELD_WRITE(_EBI_TRPD_VREG(index),_EBI_TRPD_MASK(index),_EBI_TRPD_POS(index), FlashTiming);
}
...
{
    return _SFR_FIELD_READ(_EBI_TRPD_VREG(index),_EBI_TRPD_MASK(index),_EBI_TRPD_POS(index));
}
