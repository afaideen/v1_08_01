...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _PMP_DUAL_READ_CS_ENABLE_VREG(index)

  MASKs: 
    _PMP_DUAL_READ_CS_ENABLE_MASK(index)

  POSs: 
    _PMP_DUAL_READ_CS_ENABLE_POS(index)

  LENs: 
    _PMP_DUAL_READ_CS_ENABLE_LEN(index)
...
PLIB_TEMPLATE void PMP_ReadChipSelectXEnable_Basic( PMP_MODULE_ID index , PMP_CHIP_SELECT chipSelect )
{
    _SFR_FIELD_WRITE(
                        _PMP_DUAL_READ_CS_ENABLE_VREG(index),
                        _PMP_DUAL_READ_CS_ENABLE_MASK(index),
                        ( (_PMP_DUAL_READ_CS_ENABLE_POS(index)) + (chipSelect) ),
                        1
    );
}
