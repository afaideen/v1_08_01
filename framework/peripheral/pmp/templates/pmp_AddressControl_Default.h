...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _PMP_ADDRESS_CONTROL_VREG(index)

  MASKs: 
    _PMP_ADDRESS_CONTROL_MASK(index)

  POSs: 
    _PMP_ADDRESS_CONTROL_POS(index)

  LENs: 
    _PMP_ADDRESS_CONTROL_LEN(index)
...
{
    return(_SFR_READ( _PMP_ADDRESS_CONTROL_VREG ( index ) ));
}
...
{
    _SFR_FIELD_WRITE(	_PMP_ADDRESS_CONTROL_VREG(index),
                        (3),
                        _PMP_ADDRESS_CONTROL_POS(index),
                        address);
}
