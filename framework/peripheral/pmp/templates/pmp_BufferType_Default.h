...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _PMP_INPUT_BUFFER_TYPE_VREG(index)

  MASKs: 
    _PMP_INPUT_BUFFER_TYPE_MASK(index)

  POSs: 
    _PMP_INPUT_BUFFER_TYPE_POS(index)

  LENs: 
    _PMP_INPUT_BUFFER_TYPE_LEN(index)
...
PLIB_TEMPLATE void PMP_InputBufferTypeSelect_Default( PMP_MODULE_ID index , PMP_INPUT_BUFFER_TYPE inputBuffer )
{
    switch( inputBuffer )
    {
    	case PMP_INPUT_BUFFER_SCHMITT :
		{
    	
    		_SFR_BIT_CLEAR(	_PMP_INPUT_BUFFER_TYPE_VREG(index),
							_PMP_INPUT_BUFFER_TYPE_POS(index));
    		break;
    	}
		
    	case PMP_INPUT_BUFFER_TTL :
    	{
    		_SFR_BIT_SET(	_PMP_INPUT_BUFFER_TYPE_VREG(index),
							_PMP_INPUT_BUFFER_TYPE_POS(index));
    		break;
    	}
