...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _USB_PACKET_XFER_DISABLED_VREG(index)

  MASKs: 
    _USB_PACKET_XFER_DISABLED_MASK(index)

  POSs: 
    _USB_PACKET_XFER_DISABLED_POS(index)

  LENs: 
    _USB_PACKET_XFER_DISABLED_LEN(index)
...
{
    return _SFR_BIT_READ(_USB_PACKET_XFER_DISABLED_VREG(index),
                         _USB_PACKET_XFER_DISABLED_POS(index) );
}
...
{
    SFR_TYPE  * vreg  = _USB_PACKET_XFER_DISABLED_VREG(index);
    unsigned int pos  = _U1CON_PKTDIS_TOKBUSY_POSITION;
...
{
    _SFR_BIT_SET( _USB_PACKET_XFER_DISABLED_VREG(index),
                  _USB_PACKET_XFER_DISABLED_POS(index) );
}
