...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _SQI_INIT1_CMDS_STATUS_CHECK_VREG(index)
    _SQI_INIT1_CMDS_COUNT_VREG(index)
    _SQI_INIT1_CMDS_LANE_MODE_VREG(index)
    _SQI_INIT1_COMMAND_3_VREG(index)
    _SQI_INIT1_COMMAND_2_VREG(index)
    _SQI_INIT1_COMMAND_1_VREG(index)

  MASKs:
    _SQI_INIT1_CMDS_STATUS_CHECK_MASK(index)
    _SQI_INIT1_CMDS_COUNT_MASK(index)
    _SQI_INIT1_CMDS_LANE_MODE_MASK(index)
    _SQI_INIT1_COMMAND_3_MASK(index)
    _SQI_INIT1_COMMAND_2_MASK(index)
    _SQI_INIT1_COMMAND_1_MASK(index)

  POSs:
    _SQI_INIT1_CMDS_STATUS_CHECK_POS(index)
    _SQI_INIT1_CMDS_COUNT_POS(index)
    _SQI_INIT1_CMDS_LANE_MODE_POS(index)
    _SQI_INIT1_COMMAND_3_POS(index)
    _SQI_INIT1_COMMAND_2_POS(index)
    _SQI_INIT1_COMMAND_1_POS(index)

  LENs:
    _SQI_INIT1_CMDS_STATUS_CHECK_LEN(index)
    _SQI_INIT1_CMDS_COUNT_LEN(index)
    _SQI_INIT1_CMDS_LANE_MODE_LEN(index)
    _SQI_INIT1_COMMAND_3_LEN(index)
    _SQI_INIT1_COMMAND_2_LEN(index)
    _SQI_INIT1_COMMAND_1_LEN(index)
...
PLIB_TEMPLATE void SQI_XIPControlWord3Set_Default( SQI_MODULE_ID index ,
												   ...
												   uint8_t initCmd1 )
{
	_SFR_WRITE (_SQI_INIT1_COMMAND_1_VREG(index),
				initStatCheck << 28 |
				initCmdCount << 26 |
				initCmdType << 24 |
				initCmd3 << 16 |
...
{
    return (uint32_t) _SFR_READ (_SQI_INIT1_COMMAND_1_VREG(index));
}
