...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _SQI_BURST_CONTROL_VREG(index)

  MASKs:
    _SQI_BURST_CONTROL_MASK(index)

  POSs:
    _SQI_BURST_CONTROL_POS(index)

  LENs:
    _SQI_BURST_CONTROL_LEN(index)
...
{
	_SFR_BIT_WRITE (_SQI_BURST_CONTROL_VREG(index),
					_SQI_BURST_CONTROL_POS(index),
					1);
}
