...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _SQI_CMD_IN_SDR_DATA_IN_DDR_VREG(index)

  MASKs:
    _SQI_CMD_IN_SDR_DATA_IN_DDR_MASK(index)

  POSs:
    _SQI_CMD_IN_SDR_DATA_IN_DDR_POS(index)

  LENs:
    _SQI_CMD_IN_SDR_DATA_IN_DDR_LEN(index)
...
{
	_SFR_BIT_WRITE (_SQI_CMD_IN_SDR_DATA_IN_DDR_VREG(index),
					_SQI_CMD_IN_SDR_DATA_IN_DDR_POS(index),
					1);
}
...
{
	return (bool) _SFR_BIT_READ (_SQI_CMD_IN_SDR_DATA_IN_DDR_VREG(index),
								 _SQI_CMD_IN_SDR_DATA_IN_DDR_POS(index));
}
