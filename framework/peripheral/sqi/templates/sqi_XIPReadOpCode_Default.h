...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _SQI_FLASH_READ_OPCODE_VREG(index)

  MASKs:
    _SQI_FLASH_READ_OPCODE_MASK(index)

  POSs:
    _SQI_FLASH_READ_OPCODE_POS(index)

  LENs:
    _SQI_FLASH_READ_OPCODE_LEN(index)
...
{
	_SFR_FIELD_WRITE (_SQI_FLASH_READ_OPCODE_VREG(index),
					  _SQI_FLASH_READ_OPCODE_MASK(index),
					  _SQI_FLASH_READ_OPCODE_POS(index),
					  opCode);
}
...
{
	return (uint8_t) _SFR_FIELD_READ (_SQI_FLASH_READ_OPCODE_VREG(index),
					  				  _SQI_FLASH_READ_OPCODE_MASK(index),
					  				  _SQI_FLASH_READ_OPCODE_POS(index));
}
