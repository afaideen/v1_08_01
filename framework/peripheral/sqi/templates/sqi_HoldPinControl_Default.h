...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _SQI_SQID2_AS_HOLD_VREG(index)

  MASKs:
    _SQI_SQID2_AS_HOLD_MASK(index)

  POSs:
    _SQI_SQID2_AS_HOLD_POS(index)

  LENs:
    _SQI_SQID2_AS_HOLD_LEN(index)
...
{
	_SFR_BIT_WRITE (_SQI_SQID2_AS_HOLD_VREG(index),
					_SQI_SQID2_AS_HOLD_POS(index),
					1);
}
...
{
    return (bool) _SFR_BIT_READ (_SQI_SQID2_AS_HOLD_VREG(index),
		   _SQI_SQID2_AS_HOLD_POS(index));
}
