...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _SQI_CLOCK_CONTROL_VREG(index)

  MASKs:
    _SQI_CLOCK_CONTROL_MASK(index)

  POSs:
    _SQI_CLOCK_CONTROL_POS(index)

  LENs:
    _SQI_CLOCK_CONTROL_LEN(index)
...
{
	_SFR_BIT_WRITE (_SQI_CLOCK_CONTROL_VREG(index),
					_SQI_CLOCK_CONTROL_POS(index),
					1);
}
