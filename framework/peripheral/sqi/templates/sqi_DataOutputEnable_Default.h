...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _SQI_DATA_OUTPUT_CONTROL_VREG(index)

  MASKs:
    _SQI_DATA_OUTPUT_CONTROL_MASK(index)

  POSs:
    _SQI_DATA_OUTPUT_CONTROL_POS(index)

  LENs:
    _SQI_DATA_OUTPUT_CONTROL_LEN(index)
...
PLIB_TEMPLATE void SQI_DataOutputEnableSelect_Default( SQI_MODULE_ID index , SQI_DATA_OEN dataPins )
{
	switch (dataPins)
	{
		case SQI_DATA_OEN_QUAD:
			_SFR_FIELD_WRITE (_SQI_DATA_OUTPUT_CONTROL_VREG(index),
							  _SQI_DATA_OUTPUT_CONTROL_MASK(index),
							  _SQI_DATA_OUTPUT_CONTROL_POS(index),
							  SQI_DATA_OEN_QUAD);
		break;
		case SQI_DATA_OEN_DUAL:
			_SFR_FIELD_WRITE (_SQI_DATA_OUTPUT_CONTROL_VREG(index),
							  _SQI_DATA_OUTPUT_CONTROL_MASK(index),
							  _SQI_DATA_OUTPUT_CONTROL_POS(index),
							  SQI_DATA_OEN_DUAL);
		break;
		case SQI_DATA_OEN_SINGLE:
			_SFR_FIELD_WRITE (_SQI_DATA_OUTPUT_CONTROL_VREG(index),
							  _SQI_DATA_OUTPUT_CONTROL_MASK(index),
							  _SQI_DATA_OUTPUT_CONTROL_POS(index),
							  SQI_DATA_OEN_SINGLE);
