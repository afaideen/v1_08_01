...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _SQI_RECEIVE_STATUS_VREG(index)

  MASKs:
    _SQI_RECEIVE_STATUS_MASK(index)

  POSs:
    _SQI_RECEIVE_STATUS_POS(index)

  LENs:
    _SQI_RECEIVE_STATUS_LEN(index)
...
{
	return (SQI_BD_STATE)_SFR_FIELD_READ (_SQI_RECEIVE_STATUS_VREG(index),
							_SQI_RECEIVE_STATUS_MASK(index),
							_SQI_RECEIVE_STATUS_POS(index));
}
