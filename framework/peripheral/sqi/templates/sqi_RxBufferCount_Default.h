...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _SQI_RECEIEVE_BUF_COUNT_VREG(index)

  MASKs:
    _SQI_RECEIEVE_BUF_COUNT_MASK(index)

  POSs:
    _SQI_RECEIEVE_BUF_COUNT_POS(index)

  LENs:
    _SQI_RECEIEVE_BUF_COUNT_LEN(index)
...
{
	return (uint8_t) _SFR_FIELD_READ (_SQI_RECEIEVE_BUF_COUNT_VREG(index),
									  _SQI_RECEIEVE_BUF_COUNT_MASK(index),
									  _SQI_RECEIEVE_BUF_COUNT_POS(index));
}
