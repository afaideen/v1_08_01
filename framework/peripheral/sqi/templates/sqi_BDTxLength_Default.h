...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _SQI_CURRENT_TRANSMIT_LENGTH_VREG(index)

  MASKs:
    _SQI_CURRENT_TRANSMIT_LENGTH_MASK(index)

  POSs:
    _SQI_CURRENT_TRANSMIT_LENGTH_POS(index)

  LENs:
    _SQI_CURRENT_TRANSMIT_LENGTH_LEN(index)
...
{
	return (uint8_t) _SFR_FIELD_READ (_SQI_CURRENT_TRANSMIT_LENGTH_VREG(index),
									  _SQI_CURRENT_TRANSMIT_LENGTH_MASK(index),
									  _SQI_CURRENT_TRANSMIT_LENGTH_POS(index));
}
