...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _SQI_SOFT_RESET_CONTROL_VREG(index)

  MASKs:
    _SQI_SOFT_RESET_CONTROL_MASK(index)

  POSs:
    _SQI_SOFT_RESET_CONTROL_POS(index)

  LENs:
    _SQI_SOFT_RESET_CONTROL_LEN(index)
...
{
	_SFR_BIT_WRITE (_SQI_SOFT_RESET_CONTROL_VREG(index),
					_SQI_SOFT_RESET_CONTROL_POS(index),
					1);
}
