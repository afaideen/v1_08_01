...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _SQI_DATA_LANES_VREG(index)
    _SQI_DUMMY_LANES_VREG(index)
    _SQI_MODE_LANES_VREG(index)
    _SQI_ADDRESS_LANES_VREG(index)
    _SQI_CMD_LANES_VREG(index)

  MASKs:
    _SQI_DATA_LANES_MASK(index)
    _SQI_DUMMY_LANES_MASK(index)
    _SQI_MODE_LANES_MASK(index)
    _SQI_ADDRESS_LANES_MASK(index)
    _SQI_CMD_LANES_MASK(index)

  POSs:
    _SQI_DATA_LANES_POS(index)
    _SQI_DUMMY_LANES_POS(index)
    _SQI_MODE_LANES_POS(index)
    _SQI_ADDRESS_LANES_POS(index)
    _SQI_CMD_LANES_POS(index)

  LENs:
    _SQI_DATA_LANES_LEN(index)
    _SQI_DUMMY_LANES_LEN(index)
    _SQI_MODE_LANES_LEN(index)
    _SQI_ADDRESS_LANES_LEN(index)
    _SQI_CMD_LANES_LEN(index)
...
{
	_SFR_FIELD_WRITE (_SQI_DATA_LANES_VREG(index),
					  _SQI_DATA_LANES_MASK(index),
					  _SQI_DATA_LANES_POS(index),
					  dataLanes);
	_SFR_FIELD_WRITE (_SQI_DUMMY_LANES_VREG(index),
					  _SQI_DUMMY_LANES_MASK(index),
					  _SQI_DUMMY_LANES_POS(index),
					  dummyLanes);
	_SFR_FIELD_WRITE (_SQI_MODE_LANES_VREG(index),
					  _SQI_MODE_LANES_MASK(index),
					  _SQI_MODE_LANES_POS(index),
					  modeLanes);
	_SFR_FIELD_WRITE (_SQI_ADDRESS_LANES_VREG(index),
					  _SQI_ADDRESS_LANES_MASK(index),
					  _SQI_ADDRESS_LANES_POS(index),
					  addrLanes);
	_SFR_FIELD_WRITE (_SQI_CMD_LANES_VREG(index),
					  _SQI_CMD_LANES_MASK(index),
					  _SQI_CMD_LANES_POS(index),
					  cmdLanes);
