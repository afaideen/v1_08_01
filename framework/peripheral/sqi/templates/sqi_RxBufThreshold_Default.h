...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _SQI_RECEIVE_BUFFER_THRESHOLD_VREG(index)

  MASKs:
    _SQI_RECEIVE_BUFFER_THRESHOLD_MASK(index)

  POSs:
    _SQI_RECEIVE_BUFFER_THRESHOLD_POS(index)

  LENs:
    _SQI_RECEIVE_BUFFER_THRESHOLD_LEN(index)
...
{
	_SFR_FIELD_WRITE (_SQI_RECEIVE_BUFFER_THRESHOLD_VREG(index),
					  _SQI_RECEIVE_BUFFER_THRESHOLD_MASK(index),
					  _SQI_RECEIVE_BUFFER_THRESHOLD_POS(index),
					  threshold);
}
...
{
	return (uint8_t) _SFR_FIELD_READ (_SQI_RECEIVE_BUFFER_THRESHOLD_VREG(index),
					  				  _SQI_RECEIVE_BUFFER_THRESHOLD_MASK(index),
					  				  _SQI_RECEIVE_BUFFER_THRESHOLD_POS(index));
}
