...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _SQI_TRANSMIT_BUF_RESET_VREG(index)

  MASKs:
    _SQI_TRANSMIT_BUF_RESET_MASK(index)

  POSs:
    _SQI_TRANSMIT_BUF_RESET_POS(index)

  LENs:
    _SQI_TRANSMIT_BUF_RESET_LEN(index)
...
{
	_SFR_BIT_WRITE (_SQI_TRANSMIT_BUF_RESET_VREG(index),
					_SQI_TRANSMIT_BUF_RESET_POS(index),
					1);
}
