...
/* Routines available for accessing VREGS, MASKS, POS, LEN are
  ...
  VREGs:
    _SQI_DMA_ENABLE_VREG(index)

  MASKs:
    _SQI_DMA_ENABLE_MASK(index)

  POSs:
    _SQI_DMA_ENABLE_POS(index)

  LENs:
    _SQI_DMA_ENABLE_LEN(index)
...
{
	_SFR_BIT_WRITE (_SQI_DMA_ENABLE_VREG(index),
					_SQI_DMA_ENABLE_POS(index),
					1);
}
...
{
	return (bool) _SFR_BIT_READ (_SQI_DMA_ENABLE_VREG(index),
	                   		     _SQI_DMA_ENABLE_POS(index));
}
