...
/* Routines available for accessing VREGS, MASKS, POS, LEN are 
  ...
  VREGs: 
    _IC_BUFFER_32BIT_VALUE_VREG(index)

  MASKs: 
    _IC_BUFFER_32BIT_VALUE_MASK(index)

  POSs: 
    _IC_BUFFER_32BIT_VALUE_POS(index)

  LENs: 
    _IC_BUFFER_32BIT_VALUE_LEN(index)
...
{
    return (uint32_t)(_SFR_READ(_IC_BUFFER_32BIT_VALUE_VREG(index)));
}
...
{
    return (uint16_t)(_SFR_READ(_IC_BUFFER_32BIT_VALUE_VREG(index)) & 0x0000FFFF);
}
