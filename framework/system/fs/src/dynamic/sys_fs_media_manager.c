...
(
    uint16_t diskNum,
        ...
        uint32_t *destination,//uint8_t *destination,
        uint32_t *source,//uint8_t *source,
    #else
        uint8_t *destination,
        uint8_t *source,   
    #endif
...
)
{
    ...
    else
    {
        Nop();
        Nop();
        blocksPerSector = 4096 / mediaWriteBlockSize;//512 / mediaWriteBlockSize;
        sector *= blocksPerSector;
        addr = sector;
        numSectors *= blocksPerSector;  //numSectors always 1*4096 = 4096
        len = numSectors;
        Nop();
    }
