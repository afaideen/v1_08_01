...
uint8_t cityTemperature[6];
uint8_t woeid[10] = {'2', '4', '7', '1', '3', '9', '0'}; //  Default WOEID for Phoenix AZ USA
uint8_t city[20] = {'?', 0};
uint8_t code[20] = {'3', '2', '0', '0', 0};
TCPIP_NET_HANDLE netH;
char myCDCBuffer[50];
uint    debug_breakpoint = 0;

static enum _forcast_ioClientState {
    SM_FORCAST_IO_HOME = 0,
    SM_FORCAST_IO_WAIT_ON_DNS,
    SM_FORCAST_IO_IS_CONNECTED,
    SM_FORCAST_IO_IS_SSL_CONNECTED,
    SM_FORCAST_IO_GET_CLOUD,
    SM_FORCAST_IO_GET_CLOUD_RESPONSE_CODE,
    SM_FORCAST_IO_GET_CLOUD_RESPONSE_TEMPERATURE,
    SM_FORCAST_IO_GET_CLOUD_RESPONSE_CITY,
    SM_FORCAST_IO_POST_CLOUD,
    SM_FORCAST_IO_POST_CLOUD_RESPONSE,
    SM_FORCAST_IO_UPDATE_CLOUD_DATABASE,
    SM_FORCAST_IO_UPDATE_CLOUD_DATABASE_RESPONSE,
    SM_FORCAST_IO_CLOSE,
    SM_FORCAST_IO_WAIT,
    SM_FORCAST_IO_DONE
} forcast_ioClientState = SM_FORCAST_IO_HOME;
...
#define WEBSITE "api.forecast.io"
#define GETREQUEST "forecastrss?w=%s&u=f"

//Latitude and Longitude of Phoenix in Arizona
uint8_t latitude[20] = {'3', '3', '.', '3', '0', 0};
uint8_t longitude[20] = {'-', '1', '1', '1', '.', '8', '4', 0};
//const uint8_t apiKEY [] = "731ee028d35fc9880b767679ea4da15a"; belongs to Luc
const uint8_t apiKEY []   = "3bfc218e49d109da7576749c003f2693";
...
void APP_Forcast_ioTasks()
{
    ...
    static uint32_t forcast_ioTimeOut;

    static IPV4_ADDR addr;

    switch (forcast_ioClientState) {
        case SM_FORCAST_IO_HOME:
        {
            ...
            result = TCPIP_DNS_Resolve(WEBSITE, TCPIP_DNS_TYPE_A);
...
//            putsUART2("Starting DNS Lookup\r\n");
            forcast_ioClientState = SM_FORCAST_IO_WAIT_ON_DNS;
        ...
        case SM_FORCAST_IO_WAIT_ON_DNS:
        {
            TCPIP_DNS_RESULT result = TCPIP_DNS_IsResolved(WEBSITE, &addr, IP_ADDRESS_TYPE_ANY);
//            SYS_CONSOLE_PRINT("result=%d\r\n", result);
            switch (result) {
...
//                    SYS_CONSOLE_PRINT("mySSLsocket=%d#", mySSL.socket);
//                    putsUART2("SSL Socket Open$$$");
                    if (mySSL.socket == INVALID_SOCKET)
                    {
                        //SYS_CONSOLE_MESSAGE("Could not start connection\r\n");
                        forcast_ioClientState--;
                        ;
                    }
//                    SYS_CONSOLE_MESSAGE("Starting connection$$$");
//                    putsUART2("Starting connection$$$");

                    Timer = SYS_TMR_TickCountGet();
                    forcast_ioClientState = SM_FORCAST_IO_IS_CONNECTED;
                    break;
                ...
                case TCPIP_DNS_RES_NO_IP_ENTRY://case TCPIP_DNS_RES_NO_ENTRY:
                default:
                {
        ...
        case SM_FORCAST_IO_IS_CONNECTED:
        {
...
//                SYS_CONSOLE_PRINT("mySSLsocket not connected..%d#", forcast_ioClientState);
                break;
            }
//            SYS_CONSOLE_MESSAGE("mySSLsocket connected..\r\n");
            putsUART2("mySSLsocket connected..###");
            ...
            forcast_ioClientState = SM_FORCAST_IO_IS_SSL_CONNECTED;

        }
        case SM_FORCAST_IO_IS_SSL_CONNECTED:
        {
            if (!TCPIP_TCP_IsConnected(mySSL.socket))
            {
                TCPIP_TLS_CloseConnection(&mySSL);
...
//            SYS_CONSOLE_MESSAGE("CyaSSL_connect..\r\n");
            int result = CyaSSL_connect(mySSL.ssl);
            if (result == SSL_SUCCESS)
            {
                forcast_ioClientState = SM_FORCAST_IO_GET_CLOUD;
        ...
        case SM_FORCAST_IO_GET_CLOUD:
        {
            ...
            sprintf(buffer, "GET /forecast/%s/%s,%s?units=si&exclude=minutely,hourly,daily HTTP/1.1\r\n"
                    "Host: %s\r\n"
                    //"Accept: text/plain\r\n"
                    //"Connection: close\r\n"
                    "\r\n",
                    apiKEY,latitude,longitude, WEBSITE);
//            putsUART2(buffer);
            SYS_CONSOLE_PRINT("\r\n\r\nSending data %s\r\n", buffer,woeid);
            ...
            forcast_ioClientState = SM_FORCAST_IO_GET_CLOUD_RESPONSE_CITY;

        }
            break;

        case SM_FORCAST_IO_GET_CLOUD_RESPONSE_CITY:
        {
            //char buffer[180];
            ...
            if (!TCPIP_TCP_IsConnected(mySSL.socket))
            {
                ...
                TCPIP_TLS_CloseConnection(&mySSL);
                forcast_ioClientState = SM_FORCAST_IO_WAIT;
            ...
            } else
            {
                ...
                char *ptr;
                ptr = strstr(buffer, "timezone\":\"");
                if (ptr)
                {

                    ptr += sizeof ("timezone\":\"") -1;
                    ...
                    forcast_ioClientState = SM_FORCAST_IO_GET_CLOUD_RESPONSE_TEMPERATURE; //SM_FORCAST_IO_GET_CLOUD_RESPONSE_CODE;
                } else
        ...
        case SM_FORCAST_IO_GET_CLOUD_RESPONSE_TEMPERATURE:
                {
            //char buffer[180];
            memset(buffer, 0, sizeof (buffer));
            if (!TCPIP_TCP_IsConnected(mySSL.socket))
            ...
            } else
            {
                ...
                if (ptr)
                {
                    ...
                    Nop();
                    sprintf(myCDCBuffer,"It is presently %s C in %s\r\n",cityTemperature,city);
                    putsUART2(myCDCBuffer);
                    debug_breakpoint = 1;
//                    SYS_CONSOLE_MESSAGE (myCDCBuffer);
                    
                    forcast_ioClientState = SM_FORCAST_IO_WAIT; //SM_FORCAST_IO_GET_CLOUD_RESPONSE_CODE;
