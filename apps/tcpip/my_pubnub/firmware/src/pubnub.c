...
{
    TCPIP_DNS_RESULT result = TCPIP_DNS_Resolve(p->origin, TCPIP_DNS_TYPE_A);
...
pubnub_update(struct pubnub *p)
{
    ...
    switch (p->state) {
        ...
        {
            IP_MULTI_ADDRESS addr;
            if (TCPIP_DNS_RES_OK == TCPIP_DNS_IsResolved((char*)p->origin, &addr, IP_ADDRESS_TYPE_IPV4)) {
                if (p->socket == INVALID_SOCKET || !TCPIP_TCP_IsConnected(p->socket)) {
                    p->socket = TCPIP_TCP_ClientOpen(IP_ADDRESS_TYPE_IPV4,
                            (PUBNUB_SSL && p->use_ssl) ? 443 : 80,
                            (IP_MULTI_ADDRESS*)&addr);
        ...
        case PS_CONNECT:
            ...
            if (!PUBNUB_SSL && !p->use_ssl) {
                ...
                if (PUBNUB_SSL && !PUBNUB_CYASSL && !TCPIP_TLSTCP_ClientStart(p->socket, (uint8_t*)p->origin, NULL)) {
                    break;
                }
        ...
        pubnub_subscribe_cb cb, void *cb_data)
{
    ...
    p->http_buf_len = snprintf(p->http_buf.url, sizeof(p->http_buf.url),
            "/subscribe/%s/%s/0/%s?" "%s%s" "%s%s%s" "&pnsdk=BirdPeek-%s%%2F%s",
