...
#define HTTP_APP_REDIRECTION_DELAY_TIME (3ul) /* second */
 ...
 ****************************************************************************/
        ...
        static HTTP_IO_RESULT HTTPPostCookies(HTTP_CONN_HANDLE connHandle);
        static HTTP_IO_RESULT HTTPPostSensor(HTTP_CONN_HANDLE connHandle);
    #endif
#endif
...
static void Helper_APP_RedirectionFlagSet(uintptr_t context, uint32_t currTick)
{
    ...
    SYS_CONSOLE_MESSAGE("g_redirect_signal=true");
}
...
uint8_t TCPIP_HTTP_UserAuthenticate(HTTP_CONN_HANDLE connHandle, uint8_t *cUser, uint8_t *cPass)
{
    ...
    //   in TCPIP_HTTP_GetExecute/Post or TCPIP_HTTP_Print callbacks on this value.
...
HTTP_IO_RESULT TCPIP_HTTP_GetExecute(HTTP_CONN_HANDLE connHandle)
{
    ...
    if(!memcmp(filename, "forms.htm", 9))
    {
        ...
        if(ptr)
            BSP_LEDStateSet(APP_TCPIP_LED_1, (*ptr == '1'));
            //LED2_IO = (*ptr == '1');
...
//		}
    ...
    else if(!memcmp(filename, "rly.cgi", 8))
	{
        ptr = TCPIP_HTTP_ArgGet(httpDataBuff, (const uint8_t *)"rlysw");
		...
		switch(*ptr)
		{
			case '1':
				OUT0 ^= 1;
				Delay10us(1);
				appData.config.sensor.rlysw[0].output = OUT0;
				sensor.rlysw[0].output = OUT0;
				break;
				
			case '2':
				OUT1 ^= 1;
				Delay10us(1);
				appData.config.sensor.rlysw[1].output = OUT1;
                sensor.rlysw[1].output = OUT1;
				break;
		}
		SaveAppConfig(&appData.config, 0);	
	}
#if defined(HTTP_APP_USE_WIFI)
    else if(!memcmp(filename, "wifi/scan.cgi", 13))
    {
...
#if defined(TCPIP_STACK_USE_DYNAMICDNS_CLIENT)
    if(!strcmp((char *)filename, "protect/dyndns/index.htm"))
        return HTTPPostDDNSConfig(connHandle);
#endif
    
#if defined(HTTP_APP_USE_WIFI)
    if(!memcmp(filename, "wifi/wconf.htm", 14))
        return HTTPPostWIFIConfig(connHandle);
#endif
#if defined(EAGLEEYE2014)
    if(!strcmp((char*)filename, "protect/cookies.htm"))
		return HTTPPostCookies(connHandle);
    if(!strcmp((char*)filename, "protect/sensorset.htm"))
		return HTTPPostSensor(connHandle);	
#endif
...
static HTTP_IO_RESULT HTTPPostConfig(HTTP_CONN_HANDLE connHandle)
{
    ...
    newAppConfig.httpNetData.startFlags = TCPIP_NETWORK_CONFIG_IP_STATIC;
    ...
    {
        // Read a form field name
        if(TCPIP_HTTP_PostNameRead(connHandle, httpDataBuff, 10) != HTTP_READ_OK)
        {
            bConfigFailure = true;
            Nop();
            break;
        ...
        {   // Save new static IP Address
            ...
            newAppConfig.MyIPAddr.Val = newIPAddress.Val;
        ...
        {   // Read new gateway address
            ...
            newAppConfig.MyGateway.Val = newIPAddress.Val;
        ...
        {   // Read new static subnet
            ...
            newAppConfig.MyMask.Val = newMask.Val;
        ...
        {   // Read new primary DNS server
            ...
            newAppConfig.PrimaryDNSServer.Val = newIPAddress.Val;
        }
        ...
        {   // Read new secondary DNS server
            ...
            newAppConfig.SecondaryDNSServer.Val = newIPAddress.Val;
        }
        ...
        {   // read the new MAC address
            ...
            for( i = 0; i < 6; i++ )
                newAppConfig.MyMACAddr.v[i] = newMACAddr.v[i];
        }
...
//            FormatNetBIOSName(httpDataBuff + 6);
//            TCPIP_Helper_FormatNetBIOSName(httpDataBuff + 6);
//            memcpy((void*)newAppConfig.NetBIOSName, httpDataBuff+6, 16);
        }
        ...
        {   // Read new DHCP Enabled flag
            ...
            if(httpDataBuff[6] == '1')
            {
                ...
                newAppConfig.httpNetData.startFlags = TCPIP_NETWORK_CONFIG_DHCP_CLIENT_ON;
            }
        }
        else if(!strcmp((char *)httpDataBuff, (const char *)"port"))
        { 
            newAppConfig.HTTPPort = atoi(httpDataBuff+6);
			appData.config.HTTPPort = newAppConfig.HTTPPort;
        }
        else if(!strcmp((char *)httpDataBuff, (const char *)"aport"))
        { 
            newAppConfig.androidServer.port = atoi(httpDataBuff+6);
			appData.config.androidServer.port = newAppConfig.androidServer.port;
        }
    }
//    appData.reboot = 0;
    if(bConfigFailure == false)
    {
        ...
        for(i = 20; i < 20u+16u; i++)
        {
            if(httpDataBuff[i] == ' '){
                httpDataBuff[i] = 0x00;
//                newAppConfig.NetBIOSName[i] = 0x00;
            }
        }
        strcpy(&newAppConfig.NetBIOSName, &httpNetData.nbnsName);
...
//    uint16_t redirection_delay = SYS_TMR_TickCounterFrequencyGet() * HTTP_APP_REDIRECTION_DELAY_TIME;
//    SYS_TMR_CallbackSingle(redirection_delay, 0, Helper_APP_RedirectionFlagSet);
//    SYS_CONSOLE_MESSAGE("g_redirect_signal=false\r\n");
    return HTTP_IO_DONE;
}
...
static HTTP_IO_RESULT HTTPPostDDNSConfig(HTTP_CONN_HANDLE connHandle)
{
    ...
    switch(smPost)
    {
        ...
        case SM_DDNS_READ_NAME:
            ...
            {
#if defined(EAGLEEYE2014)
                strcpy(appData.config.ddns.username, DDNSClient.Username.szRAM);
				strcpy(appData.config.ddns.password, DDNSClient.Password.szRAM);
				strcpy(appData.config.ddns.hostname, DDNSClient.Host.szRAM);
				SaveAppConfig(&appData.config, 0);
#endif
...
#if defined(EAGLEEYE2014)
            appData.config.ddns.svcid = (BYTE)atol((char*)httpDataBuff);
            TCPIP_DDNS_ServiceSet((uint8_t)appData.config.ddns.svcid );
#else
        ...
        case SM_DDNS_DONE:
            ...
            lastSuccess = true;
            strcpy((char *)httpDataBuff, "/protect/dyndns/index.htm");
            TCPIP_HTTP_CurrentConnectionStatusSet(connHandle, HTTP_REDIRECT);
            return HTTP_IO_DONE;
    }
...
void TCPIP_HTTP_Print_btn(HTTP_CONN_HANDLE connHandle, uint16_t num)
{
    ...
    switch(num)
    {
        ...
        case 1:
#if defined(EX16)
            num = BSP_SWITCH_StateGet(BSP_SWITCH_2_CHANNEL, APP_TCPIP_SWITCH_2);
        ...
        case 2:
#if defined(EX16)
            num = BSP_SWITCH_StateGet(BSP_SWITCH_3_CHANNEL, APP_TCPIP_SWITCH_3);
#elif defined (EAGLEEYE2014)
...
void TCPIP_HTTP_Print_led(HTTP_CONN_HANDLE connHandle, uint16_t num)
{
    ...
    switch(num)
    {
		...
		case 10:
			...
			break;
		case 11:
			num = appData.config.sensor.gen_adc[0].alarm_state;
			if(!appData.config.sensor.gen_adc[0].en)
				num = 0;
			break;
		case 12:
			num = appData.config.sensor.bckup_batt.alarm_state;
			if(!appData.config.sensor.bckup_batt.en)
				num = 0;
			break;
			
		case 13:
			num = appData.config.sensor.dcin.alarm_state;
			if(!appData.config.sensor.dcin.en)
				num = 0;
			break;
		case 14:
			num = appData.config.sensor.current[0].alarm_state;
			if(!appData.config.sensor.current[0].en)
				num = 0;
			break;
		default:
			num = 0;
...
void TCPIP_HTTP_Print_hellomsg(HTTP_CONN_HANDLE connHandle)
{
    ...
    ptr = appData.config.nickname;//TCPIP_HTTP_ArgGet(TCPIP_HTTP_CurrentConnectionDataBufferGet(connHandle), (const uint8_t *)"name");
    // We omit checking for space because this is the only data being written
    if(ptr != NULL)
    {
        TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"Welcome, ");
...
//extern TCPIP_NET_HANDLE    httpNetData_currNet __attribute__((persistent));
void TCPIP_HTTP_Print_reboot(HTTP_CONN_HANDLE connHandle)
{
    ...
    {   // valid data     
        ...
        if(newAppConfig.Flags.bIsDHCPEnabled == 0){
            if( TCPIP_DHCP_Disable(httpNetData.currNet) ){
                SYS_CONSOLE_MESSAGE("setip..\r\n");
                TCPIP_STACK_NetAddressSet(httpNetData.currNet, &newAppConfig.MyIPAddr, &newAppConfig.MyMask, true);
                TCPIP_STACK_NetAddressGatewaySet(httpNetData.currNet, &newAppConfig.MyGateway);
                TCPIP_STACK_NetAddressDnsPrimarySet(httpNetData.currNet, &newAppConfig.PrimaryDNSServer);
                TCPIP_STACK_NetAddressDnsSecondSet(httpNetData.currNet, &newAppConfig.SecondaryDNSServer);
            }
        }
        else
            TCPIP_DHCP_Enable(httpNetData.currNet);
        if(strcmp(TCPIP_STACK_NetBIOSName(httpNetData.currNet), &newAppConfig.NetBIOSName))
            TCPIP_STACK_NetBiosNameSet(httpNetData.currNet, &newAppConfig.NetBIOSName);
        if(memcmp(TCPIP_STACK_NetAddressMac(httpNetData.currNet), &newAppConfig.macAddress, 6) )
            TCPIP_STACK_NetAddressMacSet(httpNetData.currNet, &newAppConfig.macAddress);
...
//        httpNetData.netConfig.startFlags = newAppConfig.httpNetData.startFlags;
        // httpNetData.netConfig.startFlags should be already set;  httpNetData.netConfig.startFlags        
        httpNetData.netConfig.pMacObject = TCPIP_STACK_MACObjectGet(httpNetData.currNet);
        memcpy(&newAppConfig.httpNetData, &httpNetData, sizeof(httpNetData));
     
//        APP_TCPIP_IFModules_Disable(httpNetData.currNet);
//        TCPIP_STACK_NetDown(httpNetData.currNet);
//        TCPIP_STACK_NetUp(httpNetData.currNet, &httpNetData.netConfig);

        SaveAppConfig(&newAppConfig, 0);
        Nop();
        NVMFLASH_EraseReadWrite(&MyConfig, &httpNetData);   //ensure phy addr of MyConfig is not within the operation of program memory.
        Nop();
        appData.reboot = 1;
...
{   // This is the expected address of the board upon rebooting
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
//    Nop();
//    strcpy( AN0String,TCPIP_HTTP_CurrentConnectionDataBufferGet(connHandle) );
    ...
    if(appData.config.HTTPPort != 80)
    {
        sprintf( AN0String, ":%d", appData.config.HTTPPort );	
        TCPIP_TCP_StringPut(sktHTTP, AN0String);
    }
}
...
void TCPIP_HTTP_Print_remoteServer(HTTP_CONN_HANDLE connHandle)
{
    ...
    TCP_SOCKET sktHTTP;
    sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    TCPIP_TCP_StringPut(sktHTTP, (const uint8_t*)appData.config.host);
#endif
}
...
void TCPIP_HTTP_Print_uuid(HTTP_CONN_HANDLE connHandle)
{
    ...
    TCP_SOCKET sktHTTP;
    sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    TCPIP_TCP_StringPut(sktHTTP, (const uint8_t*)appData.config.uuid); //nextSSID
#endif
}
...
void TCPIP_HTTP_Print_show_dc(HTTP_CONN_HANDLE connHandle, uint16_t num)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    // Determine which LED
    switch(num)
    {
        case 0:
			if(appData.config.sensor.dry[0].sensorType == NONE)
		...
		case 1:
			if(appData.config.sensor.dry[1].sensorType == NONE)
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)" ");
			else
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"none");
			break;
		case 2:
			if(appData.config.sensor.dry[2].sensorType == NONE)
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)" ");
			else
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"none");
			break;
		case 3:
			if(appData.config.sensor.dry[3].sensorType == NONE)
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)" ");
			else
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"none");
			break;
...
void TCPIP_HTTP_Print_show_co(HTTP_CONN_HANDLE connHandle, uint16_t num)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    // Determine which LED
    switch(num)
    {
        case 0:
			if(appData.config.sensor.dry[0].sensorType == GAS_CO)
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"block");
			else
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"none");
			break;
		...
		case 1:
			if(appData.config.sensor.dry[1].sensorType == GAS_CO)
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"block");
			else
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"none");
			break;
		case 2:
			if(appData.config.sensor.dry[2].sensorType == GAS_CO)
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"block");
			else
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"none");
			break;
		case 3:
			if(appData.config.sensor.dry[3].sensorType == GAS_CO)
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"block");
			else
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"none");
			break;
...
void TCPIP_HTTP_Print_show_door(HTTP_CONN_HANDLE connHandle, uint16_t num)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    // Determine which LED
    switch(num)
    {
        case 0:
			if(appData.config.sensor.dry[0].sensorType == DOOR)
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"block");
			else
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"none");
			break;
		...
		case 1:
			if(appData.config.sensor.dry[1].sensorType == DOOR)
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"block");
			else
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"none");
			break;
		case 2:
			if(appData.config.sensor.dry[2].sensorType == DOOR)
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"block");
			else
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"none");
			break;
		case 3:
			if(appData.config.sensor.dry[3].sensorType == DOOR)
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"block");
			else
				TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"none");
			break;
...
void TCPIP_HTTP_Print_rlysw(HTTP_CONN_HANDLE connHandle, uint16_t num)
{
    // Determine which LED
    switch(num)
    {
...
void TCPIP_HTTP_Print_dry(HTTP_CONN_HANDLE connHandle, WORD val)
{	
	sprintf(AN0String, "%d", sensor.dry[val].alarm_state);	
    TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
	
}
void TCPIP_HTTP_Print_humid0(HTTP_CONN_HANDLE connHandle, WORD val)
{
	if(sensor.humid[0].reading<0)
		strcpy(AN0String, "-");
    else if(sensor.humid[0].reading>100)
		strcpy(AN0String, "100.0");
	else
		sprintf(AN0String, "%.1f", sensor.humid[0].reading);	
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
	
}
void TCPIP_HTTP_Print_humidTh(HTTP_CONN_HANDLE connHandle)
{	
	sprintf(AN0String,"%.1f", appData.config.sensor.humid[0].threshold);
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
...
{	
	sprintf(AN0String,"%.1f", appData.config.sensor.light[0].threshold);
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
}
void TCPIP_HTTP_Print_batteryTh(HTTP_CONN_HANDLE connHandle)
{	
	sprintf(AN0String,"%.1f", appData.config.sensor.battery[0].threshold);
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
}
void TCPIP_HTTP_Print_sensor_adc(HTTP_CONN_HANDLE connHandle)
{	
    if(appData.config.sensor.gen_adc[0].sensorType == NONE)
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"block");
	else
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"none");	
	
}
void TCPIP_HTTP_Print_adc_pir(HTTP_CONN_HANDLE connHandle)
{
    if(appData.config.sensor.gen_adc[0].sensorType == PIR)
        TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"block");
	else
        TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"none");
	
}
void TCPIP_HTTP_Print_temp(HTTP_CONN_HANDLE connHandle, WORD val)
{
	if(sensor.temperature[val].reading<120 && sensor.temperature[val].reading>-50)		
		sprintf(AN0String, "%.1f", sensor.temperature[val].reading);
	else
		strcpy(AN0String, "-");	
    TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
...
{	
	sprintf(AN0String,"%.1f", appData.config.sensor.temperature[0].threshold);	
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
}
void TCPIP_HTTP_Print_waterTh(HTTP_CONN_HANDLE connHandle)
{	
	sprintf(AN0String,"%.1f", appData.config.sensor.water[0].threshold);
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
}
void TCPIP_HTTP_Print_adcTh(HTTP_CONN_HANDLE connHandle)
{	
	sprintf(AN0String,"%.1f", appData.config.sensor.gen_adc[0].threshold);
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
}
void TCPIP_HTTP_Print_curTh(HTTP_CONN_HANDLE connHandle)
{	
	sprintf(AN0String,"%.1f", appData.config.sensor.current[0].threshold);
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
}
void TCPIP_HTTP_Print_batt0(HTTP_CONN_HANDLE connHandle)
{
	sprintf(AN0String, "%.1f", sensor.battery[0].reading);	
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
	
}
void TCPIP_HTTP_Print_water0(HTTP_CONN_HANDLE connHandle)		
{
	sprintf(AN0String, "%.1f", sensor.water[0].reading);	
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
}
void TCPIP_HTTP_Print_light0(HTTP_CONN_HANDLE connHandle)		
{
	sprintf(AN0String, "%.1f", sensor.light[0].signal);	
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
}
void TCPIP_HTTP_Print_adc0(HTTP_CONN_HANDLE connHandle)		
{
	sprintf(AN0String, "%.1f", sensor.gen_adc[0].reading);	
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
}
void TCPIP_HTTP_Print_cur0(HTTP_CONN_HANDLE connHandle)		
{
	sprintf(AN0String, "%.1f", sensor.current[0].reading);	
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
}
void TCPIP_HTTP_Print_vdc_input(HTTP_CONN_HANDLE connHandle)
{
	sprintf(AN0String, "%.1f", sensor.dcin.reading);	
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
}
void TCPIP_HTTP_Print_vbatt(HTTP_CONN_HANDLE connHandle)
{
	sprintf(AN0String, "%.1f", charger.vbatt);	
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
}
void TCPIP_HTTP_Print_chg_status(HTTP_CONN_HANDLE connHandle)	
{
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (charger.status?"ON":"OFF"));
}
void TCPIP_HTTP_Print_ichg(HTTP_CONN_HANDLE connHandle)
{
	sprintf(AN0String, "%.1f", charger.ichg);		
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
}
void TCPIP_HTTP_Print_config_port(HTTP_CONN_HANDLE connHandle)
{
	uitoa(appData.config.HTTPPort, AN0String);	
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
	return;
}
void TCPIP_HTTP_Print_androidport(HTTP_CONN_HANDLE connHandle, WORD value)
{
	if(appData.config.androidServer.port == value)
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"");	
	return;

}
static IP_ADDR currentPublicIP;

void TCPIP_HTTP_Print_Public_IPAddress(HTTP_CONN_HANDLE connHandle)
{	
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	currentPublicIP = TCPIP_DDNS_LastIPGet();// DDNSGetLastIP();//FindPublicIPAddr();		
	
    if(TCPIP_Helper_IPAddressToString(&currentPublicIP, s_buf_ipv4addr, HTTP_APP_IPV4_ADDRESS_BUFFER_SIZE) )
        TCPIP_TCP_StringPut(sktHTTP, s_buf_ipv4addr);
}
void TCPIP_HTTP_Print_status_ip(HTTP_CONN_HANDLE connHandle)
{
	if(currentPublicIP.Val)
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"block");
	else
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"none");
//	currentPublicIP.Val = FALSE;
	
}
void TCPIP_HTTP_Print_new_wanip(HTTP_CONN_HANDLE connHandle)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(TCPIP_Helper_IPAddressToString(&appData.config.WAN_IPAddr[1].my_ip, s_buf_ipv4addr, HTTP_APP_IPV4_ADDRESS_BUFFER_SIZE) )
        TCPIP_TCP_StringPut(sktHTTP, s_buf_ipv4addr);
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(TCPIP_Helper_IPAddressToString(&appData.config.WAN_IPAddr[0].my_ip, s_buf_ipv4addr, HTTP_APP_IPV4_ADDRESS_BUFFER_SIZE) )
        TCPIP_TCP_StringPut(sktHTTP, s_buf_ipv4addr);
...
void TCPIP_HTTP_Print_cookieven(HTTP_CONN_HANDLE connHandle)
{
	...
	ptr = appData.config.venue;//HTTPGetROMArg(curHTTP.data, (ROM BYTE*)"name");
	
	if(ptr)
		TCPIP_TCP_StringPut(sktHTTP, ptr);
	else
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"not set");
...
void TCPIP_HTTP_Print_cookieddns(HTTP_CONN_HANDLE connHandle)
{
	...
	ptr = appData.config.ddns.hostname;
...
void TCPIP_HTTP_Print_cookiemail(HTTP_CONN_HANDLE connHandle)
{
	...
	ptr = appData.config.email.To;//HTTPGetROMArg(curHTTP.data, (ROM BYTE*)"email");
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	strcpy(AN0String, appData.config.username);
	TCPIP_TCP_StringPut(sktHTTP, AN0String);	
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	strcpy(AN0String, appData.config.password);
	TCPIP_TCP_StringPut(sktHTTP, AN0String);	
}
static HTTP_IO_RESULT HTTPPostSensor(HTTP_CONN_HANDLE connHandle)
{
    uint8_t httpData_Val[26], httpData_Name[20], i;
    static uint8_t *ptrData, *httpDataBuff, initCheckBox = 0;
    uint32_t byteCount;
    float temp;
    ...
    if(!initCheckBox)
	{
        ...
        appData.config.sensor.temperature[0].en = 0;	
        appData.config.sensor.temperature[1].en = 0;
    ...
    //    appData.config.sensor.gen_adc[0].schedule.en = 0;	
    //    appData.config.sensor.temperature[0].schedule.en = 0;
    //    appData.config.sensor.humid[0].schedule.en = 0;
    //    appData.config.sensor.current[0].schedule.en = 0;
    //    appData.config.sensor.light[0].schedule.en = 0;
    //    appData.config.sensor.water[0].schedule.en = 0;
        for(i = 0; i < NUM_DRY_INP; i++)	
        {
            sensor.dry[i].en = 0;	
    //        appData.config.sensor.dry[i].schedule.en = 0;
            appData.config.sensor.dry[i].notify_clear = 0;	
        }
    }
    
//    httpDataBuff = TCPIP_HTTP_CurrentConnectionDataBufferGet(connHandle);
//    httpDataBuff = &httpData_Val[0];
    
    if(TCPIP_HTTP_PostNameRead(connHandle, &httpData_Name, sizeof(httpData_Val)) == HTTP_READ_INCOMPLETE)
        return HTTP_IO_NEED_DATA;    
    TCPIP_HTTP_PostValueRead(connHandle, &httpData_Val, TCPIP_HTTP_MAX_DATA_LEN);
//    SYS_CONSOLE_PRINT("Name = %s, Value = %s\r\n", httpData_Name, httpData_Val);
    byteCount = TCPIP_HTTP_CurrentConnectionByteCountGet(connHandle);
    ptrData = &httpData_Val[0];
    Nop();
    
    {
        if(!strcmp(&httpData_Name, (ROM char*)"param_humid"))
        {
            if(temp = atof(ptrData))
            {
                if(temp <= 100 && temp >=0){
                    appData.config.sensor.humid[0].threshold = temp;
                    sensor.humid[0].threshold = temp;
                }
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"cbHumid"))
        {
            if(temp = atoi(ptrData))
            {
                appData.config.sensor.humid[0].en = 1;
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"HumidAlmType"))
        {
            appData.config.sensor.humid[0].AlarmType = atoi(ptrData);
           
        }
        if(!strcmp(&httpData_Name, (ROM char*)"param_cur"))
        {
            if(temp = atof(ptrData))
            {
                if(temp <= 100 && temp >=0){
                    appData.config.sensor.current[0].threshold = temp;
                    sensor.current[0].threshold = temp;
                }
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"cbCur"))
        {
            if(temp = atoi(ptrData))
            {
                appData.config.sensor.current[0].en = 1;
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"CurAlmType"))
        {
            appData.config.sensor.current[0].AlarmType = atoi(ptrData);
           
        }
        if(!strcmp(&httpData_Name, (ROM char*)"param_light"))
        {
            if(temp = atof(ptrData))
            {
                if(temp <= 3.3 && temp >=0)	{
                    appData.config.sensor.light[0].threshold = temp;
                    sensor.light[0].threshold = temp;
                }
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"cbLight"))
        {
            if(temp = atoi(ptrData))
            {
                appData.config.sensor.light[0].en = 1;
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"LightAlmType"))
        {
            appData.config.sensor.light[0].AlarmType = atoi(ptrData);
           
        }
        if(!strcmp(&httpData_Name, (ROM char*)"param_dcin"))
        {
            if(temp = atof(ptrData))
            {
                if(temp <= 15 && temp >=0)	{
                    appData.config.sensor.dcin.threshold = temp;
                    sensor.dcin.threshold = temp;
                }
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"cbDCin"))
        {
            if(temp = atoi(ptrData))
            {
                appData.config.sensor.dcin.en = 1;
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"DCMainAlmType"))
        {
            appData.config.sensor.dcin.AlarmType = atoi(ptrData);
           
        }
        if(!strcmp(&httpData_Name, (ROM char*)"param_batt"))
        {
            if(temp = atof(ptrData))
            {
                if(temp <= 100 && temp >=0)	{
                    appData.config.sensor.battery[0].threshold = temp;
                    sensor.dcin.threshold = temp;
                }
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"cbBatt"))
        {
            if(temp = atoi(ptrData))
            {
                appData.config.sensor.battery[0].en = 1;
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"BattAlmType"))
        {
            appData.config.sensor.battery[0].AlarmType = atoi(ptrData);
           
        }
        if(!strcmp(&httpData_Name, (ROM char*)"param_bckbatt"))
        {
            if(temp = atof(ptrData))
            {
                if(temp <= 20 && temp >=0)	{
                    appData.config.sensor.bckup_batt.threshold = temp;
                    sensor.bckup_batt.threshold = temp;
                }
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"cbBckBatt"))
        {
            if(temp = atoi(ptrData))
            {
                appData.config.sensor.bckup_batt.en = 1;
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"BckBattAlmType"))
        {
            appData.config.sensor.bckup_batt.AlarmType = atoi(ptrData);
           
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"param_tpr"))
        {
            if(temp = atof(ptrData))
            {
                if(temp <= 150 && temp >= -50){
                    appData.config.sensor.temperature[0].threshold = temp;
                    sensor.humid[0].threshold = temp;
                }
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"cbTpr"))
        {
            if(temp = atoi(ptrData))
            {
                appData.config.sensor.temperature[0].en = 1;
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"TprAlmType"))
        {
            appData.config.sensor.temperature[0].AlarmType = atoi(ptrData);
           
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"param_water"))
        {
            if(temp = atof(ptrData))
            {
                if(temp <= 3.3 && temp >= 0){
                    appData.config.sensor.water[0].threshold = temp;
                    sensor.water[0].threshold = temp;
                }
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"cbWater"))
        {
            if(temp = atoi(ptrData))
            {
                appData.config.sensor.water[0].en = 1;
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"WtrAlmType"))
        {
            appData.config.sensor.water[0].AlarmType = atoi(ptrData);
           
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"param_adc"))
        {
            if(temp = atof(ptrData))
            {
                if(temp <= 5 && temp >= 0){
                    appData.config.sensor.gen_adc[0].threshold = temp;
                    sensor.gen_adc[0].threshold = temp;
                }
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"cbAdc"))
        {
            if(temp = atoi(ptrData))
            {
                appData.config.sensor.gen_adc[0].en = 1;
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"AdcAlmType"))
        {
            appData.config.sensor.gen_adc[0].AlarmType = atoi(ptrData);
           
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"AdcSensorType"))
        {
            appData.config.sensor.gen_adc[0].sensorType = atoi(ptrData);
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"cbDry1"))
        {
            if(temp = atoi(ptrData))
            {
                appData.config.sensor.dry[0].en = 1;
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"Dry1AlmType"))
        {
            appData.config.sensor.dry[0].AlarmType = atoi(ptrData);
           
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"Dry1SensorType"))
        {
            if(atoi(ptrData) == 0)
                appData.config.sensor.dry[0].sensorType = NONE;
            else if(atoi(ptrData) == 1)
                appData.config.sensor.dry[0].sensorType = GAS_CO;
            else if(atoi(ptrData) == 2)
                appData.config.sensor.dry[0].sensorType = DOOR;	
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"cbDryClr1"))
        {
            if(atoi(ptrData))
                appData.config.sensor.dry[0].notify_clear = 1;            
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"cbDry2"))
        {
            if(temp = atoi(ptrData))
            {
                appData.config.sensor.dry[1].en = 1;
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"Dry2AlmType"))
        {
            appData.config.sensor.dry[1].AlarmType = atoi(ptrData);
           
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"Dry2SensorType"))
        {
            if(atoi(ptrData) == 0)
                appData.config.sensor.dry[1].sensorType = NONE;
            else if(atoi(ptrData) == 1)
                appData.config.sensor.dry[1].sensorType = GAS_CO;
            else if(atoi(ptrData) == 2)
                appData.config.sensor.dry[1].sensorType = DOOR;	
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"cbDryClr2"))
        {
            if(atoi(ptrData))
                appData.config.sensor.dry[1].notify_clear = 1;            
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"cbDry3"))
        {
            if(temp = atoi(ptrData))
            {
                appData.config.sensor.dry[2].en = 1;
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"Dry3AlmType"))
        {
            appData.config.sensor.dry[2].AlarmType = atoi(ptrData);
           
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"Dry3SensorType"))
        {
            if(atoi(ptrData) == 0)
                appData.config.sensor.dry[2].sensorType = NONE;
            else if(atoi(ptrData) == 1)
                appData.config.sensor.dry[2].sensorType = GAS_CO;
            else if(atoi(ptrData) == 2)
                appData.config.sensor.dry[2].sensorType = DOOR;	
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"cbDryClr3"))
        {
            if(atoi(ptrData))
                appData.config.sensor.dry[2].notify_clear = 1;            
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"cbDry4"))
        {
            if(temp = atoi(ptrData))
            {
                appData.config.sensor.dry[3].en = 1;
            }
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"Dry4AlmType"))
        {
            appData.config.sensor.dry[3].AlarmType = atoi(ptrData);
           
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"Dry4SensorType"))
        {
            if(atoi(ptrData) == 0)
                appData.config.sensor.dry[3].sensorType = NONE;
            else if(atoi(ptrData) == 1)
                appData.config.sensor.dry[3].sensorType = GAS_CO;
            else if(atoi(ptrData) == 2)
                appData.config.sensor.dry[3].sensorType = DOOR;	
        }
        else if(!strcmp(&httpData_Name, (ROM char*)"cbDryClr4"))
        {
            if(atoi(ptrData))
                appData.config.sensor.dry[3].notify_clear = 1;            
        }
        if(byteCount)
            return HTTP_IO_WAITING;
    }
...
{
//	static BYTE *ptrData;
    uint8_t *httpDataBuff, httpData_Val[26], httpData_Name[12];
    uint32_t byteCount;
    ...
    TCPIP_HTTP_PostValueRead(connHandle, &httpData_Val, TCPIP_HTTP_MAX_DATA_LEN);
    byteCount = TCPIP_HTTP_CurrentConnectionByteCountGet(connHandle);
    if(!strcmp(&httpData_Name, (ROM char*)"name"))
    {
        if(*(httpDataBuff))
            strcpy(appData.config.nickname, httpDataBuff);
        Nop();
        if(byteCount)
            return HTTP_IO_WAITING;
    }
    else if(!strcmp(&httpData_Name, (ROM char*)"ven"))
    {
        if(*(httpDataBuff))
            strcpy(appData.config.venue, httpDataBuff);
        Nop();
        if(byteCount)
            return HTTP_IO_WAITING;
    }
    else if(!strcmp(&httpData_Name, (ROM char*)"email"))
    {
        if(*(httpDataBuff))
            strcpy(appData.config.email.To, httpDataBuff);
        Nop();
        if(byteCount)
            return HTTP_IO_WAITING;
    }
    else if(!strcmp(&httpData_Name, (ROM char*)"user"))
    {
        if(*(httpDataBuff))
            strcpy(appData.config.username, httpDataBuff);
        Nop();
        if(byteCount)
            return HTTP_IO_WAITING;
    }
    else if(!strcmp(&httpData_Name, (ROM char*)"pass"))
    {
        if(*(httpDataBuff))
            strcpy(appData.config.password, httpDataBuff);
        Nop();
        if(byteCount)
            return HTTP_IO_WAITING;
    }
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    sprintf(AN0String,"%.1f", appData.config.ntp.GMT_TIME);
    TCPIP_TCP_StringPut(sktHTTP, AN0String);	
}
void TCPIP_HTTP_Print_ntp_checked(HTTP_CONN_HANDLE connHandle)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(appData.config.ntp.en)
        TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"checked");
...
void TCPIP_HTTP_Print_date(HTTP_CONN_HANDLE connHandle)
{
    ...
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    TCPIP_TCP_StringPut(sktHTTP, _date_str);
...
void TCPIP_HTTP_Print_time(HTTP_CONN_HANDLE connHandle)
{
    ...
    appData.config.TimeNow = whattime;
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    TCPIP_TCP_StringPut(sktHTTP, _time_str);
    
}
void TCPIP_HTTP_Print_config_humidchecked(HTTP_CONN_HANDLE connHandle)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	if(appData.config.sensor.humid[0].en)
        TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"checked");	
}
void TCPIP_HTTP_Print_HumidAlmType(HTTP_CONN_HANDLE connHandle, WORD value)
{
	if(appData.config.sensor.humid[0].AlarmType == value)
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"");	
	return;
...
void TCPIP_HTTP_Print_CurAlmType(HTTP_CONN_HANDLE connHandle, WORD value)
{
	if(appData.config.sensor.current[0].AlarmType == value)
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"");	
	return;
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	if(appData.config.sensor.light[0].en)
        TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"checked");	
}
void TCPIP_HTTP_Print_LightAlmType1(HTTP_CONN_HANDLE connHandle)
{
	if(appData.config.sensor.light[0].AlarmType == 1)
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"");	
	return;
...
{
	if(appData.config.sensor.light[0].AlarmType == 2)
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"");	
	return;
...
{
	if(appData.config.sensor.light[0].AlarmType == 3)
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"");	
	return;
}
TCPIP_HTTP_Print_dcinTh(HTTP_CONN_HANDLE connHandle)
{
    sprintf(AN0String,"%.1f", appData.config.sensor.dcin.threshold);
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
}
...
void TCPIP_HTTP_Print_DCMainAlmType(HTTP_CONN_HANDLE connHandle, WORD value)
{
	if(appData.config.sensor.dcin.AlarmType == value)
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"");	
	return;
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	if(appData.config.sensor.battery[0].en)
        TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"checked");	
}
void TCPIP_HTTP_Print_BattAlmType(HTTP_CONN_HANDLE connHandle, WORD value)
{
	if(appData.config.sensor.battery[0].AlarmType == value)
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"");	
	return;
}
TCPIP_HTTP_Print_bckbatteryTh(HTTP_CONN_HANDLE connHandle)
{
    sprintf(AN0String,"%.1f", appData.config.sensor.bckup_batt.threshold);
	TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), AN0String);
}
...
void TCPIP_HTTP_Print_BckBattAlmType(HTTP_CONN_HANDLE connHandle, WORD value)
{
	if(appData.config.sensor.bckup_batt.AlarmType == value)
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (ROM BYTE*)"");	
	return;
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	if(appData.config.sensor.temperature[0].en)
        TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"checked");	
}
void TCPIP_HTTP_Print_config_waterchecked(HTTP_CONN_HANDLE connHandle)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	if(appData.config.sensor.water[0].en)
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"checked");
	return;
}
...
void TCPIP_HTTP_Print_config_drychecked(HTTP_CONN_HANDLE connHandle, WORD val)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	if(appData.config.sensor.dry[val].en)
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"checked");
	return;
}

void TCPIP_HTTP_Print_notify_dry_clear(HTTP_CONN_HANDLE connHandle, WORD val)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	if(appData.config.sensor.dry[val].notify_clear)
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"checked");
	return;
}
void TCPIP_HTTP_Print_TprAlmType(HTTP_CONN_HANDLE connHandle, WORD value)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	if(appData.config.sensor.temperature[0].AlarmType==value)
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"");	
	return;
	
}


void TCPIP_HTTP_Print_WtrAlmType(HTTP_CONN_HANDLE connHandle, WORD value)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	if(appData.config.sensor.water[0].AlarmType==value)
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"");	
	return;
...
void TCPIP_HTTP_Print_ADCAlmType(HTTP_CONN_HANDLE connHandle, WORD value)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	if(appData.config.sensor.gen_adc[0].AlarmType==value)
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"");	
	return;
...
void TCPIP_HTTP_Print_Dry1Sensor(HTTP_CONN_HANDLE connHandle, WORD val)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	switch(val)
	{
...
void TCPIP_HTTP_Print_Dry2Sensor(HTTP_CONN_HANDLE connHandle, WORD val)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	switch(val)
	{
...
void TCPIP_HTTP_Print_Dry3Sensor(HTTP_CONN_HANDLE connHandle, WORD val)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	switch(val)
	{
...
void TCPIP_HTTP_Print_Dry4Sensor(HTTP_CONN_HANDLE connHandle, WORD val)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	switch(val)
	{
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	if(appData.config.sensor.dry[0].AlarmType==1)
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"");	
	return;
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	if(appData.config.sensor.dry[0].AlarmType==2)
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"");	
	return;
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	if(appData.config.sensor.dry[1].AlarmType==1)
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"");	
	return;
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	if(appData.config.sensor.dry[1].AlarmType==2)
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"");	
	return;
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	if(appData.config.sensor.dry[2].AlarmType==1)
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"");	
	return;
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	if(appData.config.sensor.dry[2].AlarmType==2)
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"");	
	return;
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	if(appData.config.sensor.dry[3].AlarmType==1)
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"");	
	return;
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
	if(appData.config.sensor.dry[3].AlarmType==2)
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"selected");
	else
		TCPIP_TCP_StringPut(sktHTTP, (ROM BYTE*)"");	
	return;
