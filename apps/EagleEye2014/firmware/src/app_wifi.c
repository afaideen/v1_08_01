...
// *****************************************************************************
// *****************************************************************************
extern TCPIP_NETWORK_CONFIG    netConfig;  // configuration in the interface requested format
static TCPIP_NET_HANDLE        netH2;
/*******************************************************************************
  Function:
...
void APP_WIFI_Tasks(void)
{
    ...
    switch (s_appData.state)
    {
        case APP_MOUNT_DISK:
            ...
            {
                if(APP_TIMER_Expired(&startTick, 5)){
...
void APP_TCPIP_IFModules_Enable(TCPIP_NET_HANDLE netH)
{
    ...
    if (IS_WF_INTF(netName)) {
        ...
        if (s_app_get_param.opMode.isServer) {
            ...
            if(TCPIP_DHCPS_Enable(netH)) // start DHCP server
                SYS_CONSOLE_MESSAGE("DHCPServer on..\r\n");
            TCPIP_DNS_Disable(netH, true);
            TCPIP_DNSS_Enable(netH);
        } else {
            TCPIP_DHCPS_Disable(netH); // must stop DHCP server first
//            if(appData.config.Flags.bIsDHCPEnabled)
            {
                if(TCPIP_DHCP_Enable(netH)) // start DHCP client
